...
@section('content')
    ...
    <div class="container mt-6">
        <div class="row justify-content-center">
            <h1 class="text-center fw-max-bold text-black fs-custom mon-alt fst-italic position-relative custom-bg"><a
                    class="px-0 text-black">Những chia sẻ từ<br class="d-none d-md-block"> những trải nghiệm vô giá!</a></h1>
            <p class="mt-3 text-sm-start text-md-center text-black fs-4">Nguồn cảm hứng đến từ những câu chuyện. Đó là những
                gì<br class="d-none d-md-block"> Organic Marketing vẫn đang và sẽ tiếp tục chia sẻ nhiều hơn nữa.
            </p>
        </div>
                    ...
                    <div class="my-4 row m-0">
                        <div class="col-12">
                            <p class="fs-3 fw-bold mb-0 ">
                                ...
                                <a class="btn fs-4 fst-italic py-0" href="{{ route('posts.show', $post->slug) }}"
                                    role="button">Xem thêm</a>
                            </p>
                            <div class="d-flex align-items-center flex-wrap mb-1">
                                <a class="btn fs-4 fst-italic text-start py-0 me-2"
                                    href="{{ route('posts.index', ['author' => $post->author->slug]) }}"
                                    role="button">#{{ $post->author->name }}</a>
                            ...
                            <div class="d-flex flex-wrap ps-0">
                                @foreach ($post->topics as $topic)
                                    <a class="btn fs-4 fst-italic text-start py-0"
                                        href="{{ route('posts.index', ['topic' => $topic->slug]) }}"
                                        role="button">#{{ $topic->name }}</a>
                                @endforeach
                                @foreach ($post->industries as $industry)
                                    <a class="btn fs-4 fst-italic text-start py-0"
                                        href="{{ route('posts.index', ['industry' => $industry->slug]) }}"
                                        role="button">#{{ $industry->name }}</a>
                                @endforeach
                            </div>
                        </div>
                    </div>
